...
const storage = multer.diskStorage({
  destination: async function (req, file, cb) {
    try {
      ...
      const result = await pool.query('SELECT ruta FROM carpetas WHERE id = $1', [carpetaId]);
...
router.get('/carpetas', verifyToken, async (req, res) => {
  try {
    const carpetas = await pool.query('SELECT * FROM carpetas ORDER BY nombre_carpeta ASC');
...
router.post('/carpetas', verifyToken, async (req, res) => {
  try {
    ...
    if (!nombre_carpeta) return res.status(400).json({ error: 'El nombre es obligatorio.' });

    const rutaFisica = path.join(RUTA_DOCS_SUELTOS, nombre_carpeta);

    // 1. Crear en el disco duro si no existe
    if (fs.existsSync(rutaFisica)) {
      return res.status(400).json({ error: 'Ya existe una carpeta con ese nombre físicamente.' });
    }
    ...
    const nuevaCarpeta = await pool.query(insertQuery, [nombre_carpeta, rutaFisica]);

    res.status(201).json({ message: 'Carpeta creada', carpeta: nuevaCarpeta.rows[0] });
  } catch (error) {
    if (error.code === '23505') { // Código de error de Postgres para UNIQUE
      return res.status(400).json({ error: 'Ese nombre de carpeta ya está registrado.' });
    }
...
router.delete('/carpetas/:id', verifyToken, async (req, res) => {
  try {
    ...
    const carpetaDB = await pool.query('SELECT ruta FROM carpetas WHERE id = $1', [id]);
    if (carpetaDB.rows.length === 0) return res.status(404).json({ error: 'Carpeta no encontrada.' });

    const rutaFisica = carpetaDB.rows[0].ruta;

    // 1. Verificar si hay archivos en la base de datos
    const docsAsociados = await pool.query('SELECT id FROM doc_carpetas WHERE carpeta_id = $1 AND estado_doc = true', [id]);
    if (docsAsociados.rows.length > 0) {
      return res.status(400).json({ error: 'No puedes borrar una carpeta que contiene documentos.' });
    }
    ...
    if (fs.existsSync(rutaFisica)) {
      try {
        ...
        return res.status(400).json({ error: 'La carpeta física contiene archivos no registrados.' });
      }
    }
...
router.put('/carpetas/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const carpetaAnterior = await client.query('SELECT * FROM carpetas WHERE id = $1', [id]);
    ...
    if (fs.existsSync(rutaNueva)) {
      return res.status(400).json({ error: 'Ya existe una carpeta física con ese nuevo nombre.' });
    }
    ...
    res.json({ message: 'Carpeta renombrada', carpeta: carpetaActualizada.rows[0] });

  } catch (error) {
    await client.query('ROLLBACK');
    if (error.message === 'NOT_FOUND') return res.status(404).json({ error: 'Carpeta no existe.' });
    if (error.code === '23505') return res.status(400).json({ error: 'El nombre ya existe.' });

    console.error(error);
    res.status(500).json({ error: 'Error al renombrar la carpeta y sus archivos.' });
  } finally {
    client.release();
  }
});
  ...
  upload.single('archivo')(req, res, async function (err) {
    ...
    if (err) {
      if (err.message === 'ARCHIVO_DUPLICADO') {
        return res.status(400).json({ error: 'El archivo ya existe en esta carpeta.' });
      }
      if (err.message === 'CARPETA_NO_ENCONTRADA') {
        return res.status(404).json({ error: 'La carpeta destino no existe en la base de datos.' });
      }
    ...
    try {
      ...
      try {
        ...
        const documentoFinal = await client.query(updateQuery, [nombreConId, rutaFisicaNueva, nuevoId]);
      ...
      res.status(500).json({ error: 'Error interno al registrar el documento en el sistema.' });
    }
  });
});
...
router.post('/documentos/:id/compartir', verifyToken, async (req, res) => {
  try {
    ...
    if (!usuarios_ids || !Array.isArray(usuarios_ids) || usuarios_ids.length === 0) {
      return res.status(400).json({ error: 'Debes enviar una lista de usuarios.' });
    }

    // 1. VERIFICACIÓN CRÍTICA: ¿Existe el documento en doc_carpetas?
    const docCheck = await pool.query('SELECT id FROM doc_carpetas WHERE id = $1', [id]);
    ...
    res.json({ message: `Documento compartido con ${usuarios_ids.length} usuario(s).` });
    
  } catch (error) {
    console.error('Error al compartir el documento:', error);
    res.status(500).json({ error: 'Error interno al intentar compartir el documento.' });
  }
});
...
router.post('/documentos/:id/vincular-caso', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const docQuery = await client.query('SELECT * FROM doc_carpetas WHERE id = $1', [docSueltoId]);
    ...
    const casoQuery = await client.query('SELECT expediente_id FROM casos WHERE caso_id = $1', [caso_id]);
    ...
    const carpetaExpediente = path.join(RUTA_DESTINO_BASE_CASOS, anioActual, expediente_id);
    ...
    await client.query('UPDATE documentos SET nombre = $1, url_archivo = $2 WHERE id = $3',
    ...
    await client.query('UPDATE doc_carpetas SET caso_id = $1 WHERE id = $2', [caso_id, docSueltoId]);

    await client.query('COMMIT');
    res.json({ message: 'Documento vinculado y copiado al expediente exitosamente.' });

  } catch (error) {
    await client.query('ROLLBACK');
    if (error.message === 'DOC_NOT_FOUND') return res.status(404).json({ error: 'Documento suelto no encontrado.' });
    if (error.message === 'CASE_NOT_FOUND') return res.status(404).json({ error: 'El caso especificado no existe.' });
    console.error(error);
...
router.delete('/documentos/:id', verifyToken, async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('UPDATE doc_carpetas SET estado_doc = false WHERE id = $1 RETURNING id', [id]);
...
// ==========================================
router.post('/carpetas/:carpeta_id/documentos/blanco', verifyToken, async (req, res) => {
    const client = await pool.connect();
    ...
    try {
        ...
        if (!nombreArchivo || !tipoPlantilla) {
            return res.status(400).json({ error: 'El nombre del archivo y el tipo de plantilla son obligatorios.' });
        }

        await client.query('BEGIN'); // Iniciamos transacción

        // 1. BUSCAR LA CARPETA EN LA BASE DE DATOS
        const carpetaQuery = await client.query('SELECT ruta FROM carpetas WHERE id = $1', [carpeta_id]);
        ...
        if (!fs.existsSync(templatePath)) {
            await client.query('ROLLBACK');
            return res.status(500).json({ error: 'El archivo de plantilla no existe en el servidor.' });
        }
        ...
        res.status(500).json({ error: 'Error interno al generar el documento en blanco.' });
    } finally {
        client.release();
    }
});
...
router.get('/files/:fileId', async (req, res) => {
    try {
        ...
        const docQuery = await pool.query('SELECT nombre, ruta_archivo, subido_por_id FROM doc_carpetas WHERE id = $1', [fileId]);
