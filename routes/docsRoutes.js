...
router.post("/:id/crearDocumento", verifyToken, async (req, res) => {
  try {
    ...
    const carpetaDinamica = path.join(
      RUTA_DESTINO_BASE,
      `${anio}/${expedienteId}`,
    );
    ...
    const documentoFinal = await pool.query(updateQuery, [
      nombreConId,
      rutaFisicaNueva,
      nuevoId,
    ]);
    ...
    res
      .status(500)
      .json({ error: "Error al generar la documentación en blanco" });
  }
});
...
router.get("/:id/documentacion", verifyToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener la documentación del caso" });
  }
});
    ...
    if (err) {
      if (err.message === "ARCHIVO_DUPLICADO") {
        return res.status(400).json({
          error: "El archivo que quiere subir ya existe en este expediente.",
        });
      }
...
router.post("/:id/nueva_version", verifyToken, (req, res) => {
  upload.single("archivo")(req, res, async function (err) {
    if (!req.file) {
      return res
        .status(400)
        .json({ error: "Debes adjuntar el nuevo documento." });
    }
    ...
    try {
      ...
      const queryActual = await pool.query(
      ...
      if (
        versionQuery.rows.length > 0 &&
        versionQuery.rows[0].version_doc != null
      ) {
      ...
      const rutaFisicaViejaArchivada = path.join(
        carpetaCorrecta,
        nombreViejoArchivado,
      );
      ...
      res.status(500).json({
        error: "Error interno al actualizar la versión del documento.",
      });
    }
  });
});
...
router.delete("/:id/eliminar", verifyToken, async (req, res) => {
  try {
    ...
    const resultadoBusqueda = await pool.query(
      "SELECT url_archivo, nombre, caso_id FROM documentos WHERE id = $1",
      [id],
    );
    ...
    if (resultadoBusqueda.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "El documento no existe o ya fue eliminado." });
    }
    ...
    await pool.query("UPDATE documentos SET estado_doc = false WHERE id = $1", [
      id,
    ]);
